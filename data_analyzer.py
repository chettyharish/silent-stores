...
def data_analyzer(start_time, bench, processor, l1_size, l1_assoc, line_size):
    ...
    opstring = "\n".join(
        [str(x) for x in sorted(rle_list, key=itemgetter(2), reverse=True)[0:50]])
    outputfile = open(
...
def script_runner():
    ...
    for l1_size in [2, 4, 8, 16, 32, 64]:
        for l1_assoc in [1, 2, 4, 8, 16]:
            for line_size in [64,128,256]:
                ...
                print(build_cmd)
                data_analyzer(
                    start_time, bench, processor, l1_size, l1_assoc, line_size)
                print()
    return
